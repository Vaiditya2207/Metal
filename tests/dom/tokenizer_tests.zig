...
test "tag:01 div" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<div>")).type);
}
test "tag:02 span" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<span>")).type);
}
test "tag:03 p" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<p>")).type);
}
test "tag:04 a" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<a>")).type);
}
test "tag:05 h1" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<h1>")).type);
}
test "tag:06 h2" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<h2>")).type);
}
test "tag:07 h3" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<h3>")).type);
}
test "tag:08 h4" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<h4>")).type);
}
test "tag:09 h5" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<h5>")).type);
}
test "tag:10 h6" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<h6>")).type);
}
test "tag:11 ul" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<ul>")).type);
}
test "tag:12 ol" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<ol>")).type);
}
test "tag:13 li" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<li>")).type);
}
test "tag:14 table" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<table>")).type);
}
test "tag:15 tr" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<tr>")).type);
}
test "tag:16 td" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<td>")).type);
}
test "tag:17 th" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<th>")).type);
}
test "tag:18 form" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<form>")).type);
}
test "tag:19 input" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<input>")).type);
}
test "tag:20 button" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<button>")).type);
}
...
test "tag:21 nav" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<nav>")).type);
}
test "tag:22 header" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<header>")).type);
}
test "tag:23 footer" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<footer>")).type);
}
test "tag:24 main" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<main>")).type);
}
test "tag:25 section" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<section>")).type);
}
test "tag:26 article" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<article>")).type);
}
test "tag:27 aside" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<aside>")).type);
}
test "tag:28 textarea" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<textarea>")).type);
}
test "tag:29 select" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<select>")).type);
}
test "tag:30 option" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<option>")).type);
}
test "tag:31 thead" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<thead>")).type);
}
test "tag:32 tbody" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<tbody>")).type);
}
test "tag:33 strong" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<strong>")).type);
}
test "tag:34 em" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<em>")).type);
}
test "tag:35 pre" {
    try std.testing.expectEqual(TokenType.start_tag, (try firstToken("<pre>")).type);
}
...
test "xss:92 script alert" {
    try mustNotCrash("<script>alert(1)</script>");
}
test "xss:93 img onerror" {
    try mustNotCrash("<img src=x onerror=alert(1)>");
}
test "xss:94 svg onload" {
    try mustNotCrash("<svg/onload=alert(1)>");
}
test "xss:95 details ontoggle" {
    try mustNotCrash("<details open ontoggle=alert(1)>");
}
test "xss:96 javascript href" {
    try mustNotCrash("<a href=\"javascript:alert(1)\">");
}
test "xss:97 iframe javascript" {
    try mustNotCrash("<iframe src=\"javascript:alert(1)\">");
}
test "xss:98 body onload" {
    try mustNotCrash("<body onload=alert(1)>");
}
test "xss:99 input onfocus" {
    try mustNotCrash("<input autofocus onfocus=alert(1)>");
}
test "xss:100 video source onerror" {
    try mustNotCrash("<video><source onerror=alert(1)>");
}
test "xss:101 css expression" {
    try mustNotCrash("<p style=\"width: expression(alert(1));\">");
}
test "xss:102 math xlink" {
    try mustNotCrash("<math><maction xlink:href=\"javascript:alert(1)\">");
}
test "xss:103 form action js" {
    try mustNotCrash("<form action=\"javascript:alert(1)\">");
}
test "xss:104 button onfocus" {
    try mustNotCrash("<button onfocus=alert(1) autofocus>");
}
test "xss:105 isindex formaction" {
    try mustNotCrash("<isindex formaction=\"javascript:alert(1)\" type=submit>");
}
test "xss:106 object data js" {
    try mustNotCrash("<object data=\"javascript:alert(1)\">");
}
test "xss:107 template injection" {
    try mustNotCrash("<%alert(1)%>");
}
test "xss:108 xml processing" {
    try mustNotCrash("<?alert(1)?>");
}
test "xss:109 onmouseover" {
    try mustNotCrash("<a onmouseover=\"alert(1)\">XSS</a>");
}
test "xss:110 base href js" {
    try mustNotCrash("<base href=\"javascript:alert(1)//\">");
}
...
test "obfusc:111 null in tag" {
    try mustNotCrash("<\x00script>alert(1)</script>");
}
test "obfusc:112 null mid tag" {
    try mustNotCrash("<scr\x00ipt>alert(1)</script>");
}
test "obfusc:113 spaces in tag" {
    try mustNotCrash("<s c r i p t>alert(1)</script>");
}
test "obfusc:114 vertical tab" {
    try mustNotCrash("<script\x0b>alert(1)</script>");
}
test "obfusc:115 form feed" {
    try mustNotCrash("<script\x0c>alert(1)</script>");
}
test "obfusc:116 slash in tag" {
    try mustNotCrash("<script/x>alert(1)</script>");
}
test "obfusc:117 double encoding" {
    try mustNotCrash("&amp;lt;script&amp;gt;alert(1)&amp;lt;/script&amp;gt;");
}
test "obfusc:118 mixed null bytes" {
    try mustNotCrash("<div\x00class=\"a\">");
}
...
test "bypass:119 dq no space" {
    try mustNotCrash("<div title=\"x\"onmouseover=alert(1)>");
}
test "bypass:120 sq no space" {
    try mustNotCrash("<div title='x'onmouseover=alert(1)>");
}
test "bypass:121 unquoted space" {
    try mustNotCrash("<div title=x onmouseover=alert(1)>");
}
test "bypass:122 reverse order" {
    try mustNotCrash("<div onmouseover=\"alert(1)\"title=x>");
}
...
test "malform:123 multiple lt" {
    try mustNotCrash("<<<<<<");
}
test "malform:124 multiple gt" {
    try mustNotCrash(">>>>>>");
}
test "malform:125 space slash" {
    try mustNotCrash("< / >");
}
test "malform:126 slash class" {
    try mustNotCrash("<p/class=a>");
}
test "malform:127 empty attr" {
    try mustNotCrash("<div id= >");
}
test "malform:128 bad comment" {
    try mustNotCrash("<!- ->");
}
test "malform:129 short comment" {
    try mustNotCrash("<!--->");
}
test "malform:130 unclosed comment" {
    try mustNotCrash("<!--");
}
test "malform:131 bare doctype" {
    try mustNotCrash("<!DOCTYPE");
}
test "malform:132 doctype space" {
    try mustNotCrash("<!DOCTYPE >");
}
test "malform:133 lt amp" {
    try mustNotCrash("<&");
}
test "malform:134 quotes" {
    try mustNotCrash("<' ' \" \"");
}
test "malform:135 whitespace tag" {
    try mustNotCrash("<div\t\n \r>");
}
test "malform:136 control chars" {
    try mustNotCrash("<a\x01\x02\x03>");
}
...
test "fuzz:141 seed_0" {
    try fuzzRun(0);
}
test "fuzz:142 seed_1" {
    try fuzzRun(1);
}
test "fuzz:143 seed_42" {
    try fuzzRun(42);
}
test "fuzz:144 seed_100" {
    try fuzzRun(100);
}
test "fuzz:145 seed_255" {
    try fuzzRun(255);
}
test "fuzz:146 seed_1000" {
    try fuzzRun(1000);
}
test "fuzz:147 seed_9999" {
    try fuzzRun(9999);
}
test "fuzz:148 seed_12345" {
    try fuzzRun(12345);
}
test "fuzz:149 seed_65535" {
    try fuzzRun(65535);
}
test "fuzz:150 seed_99999" {
    try fuzzRun(99999);
}
...
test "rawtext:151 script content not parsed as HTML" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    var tok = Tokenizer.init(arena.allocator(), "<script>var x = 1 < 2;</script>");

    const t1 = try tok.next();
    try std.testing.expectEqual(TokenType.start_tag, t1.type);
    try std.testing.expectEqualStrings("script", t1.tag_name.?);

    const t2 = try tok.next();
    try std.testing.expectEqual(TokenType.character, t2.type);
    try std.testing.expectEqualStrings("var x = 1 < 2;", t2.data.?);

    const t3 = try tok.next();
    try std.testing.expectEqual(TokenType.end_tag, t3.type);
    try std.testing.expectEqualStrings("script", t3.tag_name.?);
}

test "rawtext:152 style content not parsed as HTML" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    var tok = Tokenizer.init(arena.allocator(), "<style>div > p { color: red; }</style>");

    const t1 = try tok.next();
    try std.testing.expectEqual(TokenType.start_tag, t1.type);
    try std.testing.expectEqualStrings("style", t1.tag_name.?);

    const t2 = try tok.next();
    try std.testing.expectEqual(TokenType.character, t2.type);
    try std.testing.expectEqualStrings("div > p { color: red; }", t2.data.?);

    const t3 = try tok.next();
    try std.testing.expectEqual(TokenType.end_tag, t3.type);
    try std.testing.expectEqualStrings("style", t3.tag_name.?);
}

test "rawtext:153 empty script" {
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    var tok = Tokenizer.init(arena.allocator(), "<script></script>");

    const t1 = try tok.next();
    try std.testing.expectEqualStrings("script", t1.tag_name.?);

    const t2 = try tok.next();
    try std.testing.expectEqual(TokenType.end_tag, t2.type);
    try std.testing.expectEqualStrings("script", t2.tag_name.?);
}
