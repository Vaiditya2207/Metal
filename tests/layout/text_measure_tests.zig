...
test "empty string returns zero width" {
    const result = text_measure.measureTextWidth("", 16.0, 400.0);
    try std.testing.expectEqual(@as(f32, 0.0), result);
}

test "fallback returns length times eight" {
    const result = text_measure.measureTextWidth("hello", 16.0, 400.0);
...
test "setMeasureFn overrides measurement" {
    const double_width = struct {
        fn measure(text: []const u8, font_size: f32, _: f32) f32 {
            _ = font_size;
    ...
    const result = text_measure.measureTextWidth("ab", 16.0, 400.0);
...
test "custom measure function receives font size" {
    ...
    const result = text_measure.measureTextWidth("any", 24.0, 400.0);
