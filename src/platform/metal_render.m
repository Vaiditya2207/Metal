...
void *create_render_pipeline(void *device_ptr) {
  @autoreleasepool {
    ...
    // Enable Alpha Blending for Rects
    ...
    vDesc.attributes[1].offset = 16; // Matches Zig RectVertex with _pad
...
void set_projection(void *frame_context, float width, float height) {
  ...
  id<MTLRenderCommandEncoder> encoder =
      (__bridge id<MTLRenderCommandEncoder>)ctx->renderEncoder;
  simd_float4x4 projection = {.columns[0] = {2.0f / width, 0, 0, 0},
...
void batch_solid_rects(void *frame_context, void *device_ptr,
  ...
  size_t byte_count = vertex_count * 32;
  id<MTLBuffer> buffer =
  ...
  // NSLog(@"[Metal] Drew %d vertices (Rect)", vertex_count);
}
...
void set_scissor_rect(void *frame_context, float x, float y, float w, float h,
  ...
  rect.width = (NSUInteger)fmaxf(1.0f, w);
  rect.height = (NSUInteger)fmaxf(1.0f, h);
