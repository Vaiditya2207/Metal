...
- (void)mouseUp:(NSEvent *)event {
  [self handleMouseEvent:event type:EVENT_MOUSE_UP];
}
- (void)mouseMoved:(NSEvent *)event {
  [self handleMouseEvent:event type:EVENT_MOUSE_MOVED];
}
...
- (void)handleKeyEvent:(NSEvent *)event type:(EventType)type {
  ...
  if (type == EVENT_KEY_DOWN && [event characters] &&
      [[event characters] length] > 0) {
    strncpy(e.characters, [[event characters] UTF8String],
            sizeof(e.characters) - 1);
    e.characters[sizeof(e.characters) - 1] = '\0';
  }
...
- (void)keyDown:(NSEvent *)event {
  [self handleKeyEvent:event type:EVENT_KEY_DOWN];
}
- (void)keyUp:(NSEvent *)event {
  [self handleKeyEvent:event type:EVENT_KEY_UP];
}
...
void *create_event_metal_view(void *window, void *device) {
  ...
  MetalEventView *view =
      [[MetalEventView alloc] initWithFrame:win.contentView.frame device:dev];
  view.paused = NO;
...
void terminate_application(void) { [NSApp terminate:nil]; }
