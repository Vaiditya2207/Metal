...
"    TextVertexOut out;\n"
"    out.position = uniforms.projection * float4(vertices[vid].position, 0.0, 1.0);\n"
...
void *create_text_pipeline(void *device_ptr) {
    @autoreleasepool {
        id<MTLDevice> device = (__bridge id<MTLDevice>)device_ptr;
        NSError *error = nil;
        id<MTLLibrary> library = [device newLibraryWithSource:[NSString stringWithUTF8String:text_shader_source] options:nil error:&error];
...
void draw_text_quad(void *fc_ptr, void *tex_ptr, float x, float y, float w, float h, float ux, float uy, float uw, float uh, float r, float g, float b, float a) {
...
float measure_text_width(const char *text, int len, float font_size) {
    ...
    CFDictionaryRef attrs = CFDictionaryCreate(
        kCFAllocatorDefault,
        (const void **)&(CFStringRef){kCTFontAttributeName},
        (const void **)&ct_font, 1,
        &kCFTypeDictionaryKeyCallBacks,
        &kCFTypeDictionaryValueCallBacks);
    CFAttributedStringRef attr_str = CFAttributedStringCreate(
